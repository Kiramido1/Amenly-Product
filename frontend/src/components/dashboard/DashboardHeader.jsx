...
const DashboardHeader = () => {
  ...
  const { user } = useAuth()
  // Gate the admin link on the logged-in user's role directly.
      ...
      <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          {isAdmin && (
            <Link
              to="/admin"
              className="group flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-white/[0.03] hover:bg-white/[0.06] border border-white/10 hover:border-amenly-light/40 transition-all duration-300 text-white/70 hover:text-white text-[11px] font-medium backdrop-blur-sm"
            >
