...
const RegulationTracker = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [frameworksRes, complianceRes] = await Promise.allSettled([
        listFrameworks(),
      ...
      if (frameworksRes.status === 'fulfilled') {
        setFrameworks(frameworksRes.value.data?.frameworks || [])
      }
      if (complianceRes.status === 'fulfilled') {
        setFrameworkScores(complianceRes.value.data?.framework_scores || {})
      }
    }
  ...
  const regulations = useMemo(() => {
    return frameworks.map((framework) => {
      const score = Math.round(frameworkScores[framework.name] || 0)
      const status = score >= 85 ? 'Compliant' : score >= 50 ? 'Partial' : 'Non-Compliant'
      const region = (framework.region || framework.category || 'global').toLowerCase()

      return {
        id: framework.id,
        country: region.includes('eu') || region.includes('europe') ? 'eu' : 'global',
        label: framework.region || framework.name,
        law: `${framework.name}${framework.version ? ` ${framework.version}` : ''}`,
        status,
        progress: score,
        requirements: [
          {
          ...
          {
            name: framework.category?.replaceAll('_', ' ') || 'Framework controls',
            status: score >= 85 ? 'compliant' : score >= 50 ? 'partial' : 'non-compliant',
            score,
          },
        ],
        lastAudit: 'From assessments',
        nextAudit: 'Continuous',
      }
    })
  }, [frameworkScores, frameworks])

  const visibleRegulations = regulations.length ? regulations : [{
    id: 'empty',
    country: 'global',
    label: 'No Frameworks',
    law: 'Add or assign frameworks to this organization',
    status: 'Partial',
    progress: 0,
    requirements: [{ name: 'Framework coverage', status: 'partial', score: 0 }],
    lastAudit: '-',
    nextAudit: '-',
  }]
    ...
    >
      ...
      <div className="px-4 sm:px-5 py-3.5 border-b border-white/10 bg-gradient-to-r from-black/40 to-transparent">
        <div className="flex items-center gap-3">
          <motion.div 
            className="w-9 h-9 rounded-lg bg-gradient-to-br from-amenly-light/20 to-amenly-medium/20 border border-amenly-light/30 flex items-center justify-center backdrop-blur-sm"
              ...
              >
                <div className="flex items-center justify-between mb-2.5">
                  <div className="flex items-center gap-2.5">
                    <CountryIcon country={reg.country || reg.id} className="flex-shrink-0" />
                    <div>
