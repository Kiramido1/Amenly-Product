...
const AssetListView = () => {
  ...
  useEffect(() => {
    const fetchAssets = async () => {
      try {
        const data = await getAssetsDashboard()
        if (data.success && data.data && data.data.assets) {
          setAssets(data.data.assets.map((asset) => {
            const riskScore = Math.min(100, (asset.risk_count || 0) * 20)
            return {
              ...asset,
              type: toDisplayAssetType(asset.type),
              status: asset.criticality === 'high' || asset.criticality === 'critical'
                ? 'critical'
                : asset.criticality === 'medium'
                ? 'warning'
                : 'secure',
              risk_score: riskScore,
              compliance_score: Math.max(0, 100 - riskScore),
              department: asset.properties?.department || 'Unassigned',
            }
          }))
        }
      } catch (err) {
