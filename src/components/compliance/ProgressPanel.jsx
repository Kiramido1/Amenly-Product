...
const ProgressPanel = ({ session, answers, currentStep, questionIndex, isComplete }) => {
  const framework = useMemo(() => FRAMEWORKS.find(f => f.id === session.framework), [session.framework])
  const questions = useMemo(() => SECURITY_QUESTIONS[session.framework] || [], [session.framework])
  ...
  const recommendations = useMemo(() => {
    ...
    return getRecommendations(session.framework, answers)
  }, [isComplete, session.framework, answers])
