  ...
  const getAIRecommendation = () => {
    if (asset.status === 'critical') {
      return {
        ...
        message: `Apply security patches immediately. ${asset.vulnerabilities.length} critical vulnerabilities detected. Isolate asset from production network until remediated.`,
        color: '#ef4444',
        badge: 'CRITICAL',
      }
      ...
      <div className="p-4 rounded-xl bg-gradient-to-br from-red-500/10 to-gray-500/10 border border-red-500/20 backdrop-blur-xl">
        <div className="flex items-center justify-between mb-3">
          ...
          <motion.span
            ...
            style={{
              backgroundColor: asset.vulnerabilities.length > 2 ? 'rgba(239,68,68,0.2)' : asset.vulnerabilities.length > 0 ? 'rgba(148,163,184,0.2)' : 'rgba(16,185,129,0.2)',
              color: asset.vulnerabilities.length > 2 ? '#ef4444' : asset.vulnerabilities.length > 0 ? '#94A3B8' : '#10b981',
              border: `1px solid ${asset.vulnerabilities.length > 2 ? 'rgba(239,68,68,0.3)' : asset.vulnerabilities.length > 0 ? 'rgba(148,163,184,0.3)' : 'rgba(16,185,129,0.3)'}`,
            }}
            animate={{
              scale: asset.vulnerabilities.length > 2 ? [1, 1.05, 1] : 1,
            }}
            transition={{
              duration: 2,
              repeat: asset.vulnerabilities.length > 2 ? Infinity : 0,
            }}
          >
            {asset.vulnerabilities.length} found
          </motion.span>
        </div>
