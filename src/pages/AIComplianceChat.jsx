...
const AIComplianceChat = () => {
  ...
  const [session, setSession] = useState({ companyName: '', industry: '', companySize: '', region: '', framework: '' })
  ...
  const handleStart = useCallback(() => setStep(STEPS.COMPANY_PROFILE), [])
  ...
  const handleFrameworkSelect = useCallback((fw) => {
    setSession(prev => ({ ...prev, framework: fw.id }))
  ...
  const handleRestart = useCallback(() => {
    ...
    setSession({ companyName: '', industry: '', companySize: '', region: '', framework: '' })
    setAnswers({})
    setQuestionIndex(0)
    setIsComplete(false)
    setPanelOpen(false)
                ...
                <Suspense fallback={<LoadingFallback />}>
                  <AnimatePresence mode="wait">
                    ...
                    {step === STEPS.COMPANY_PROFILE && (
                      <StepForm key="company" onComplete={handleCompanyComplete} onBack={handleBack} />
                    )}
                    {step === STEPS.FRAMEWORK && (
                      <FrameworkSelector key="framework" onSelect={handleFrameworkSelect} selected={session.framework} onBack={handleBack} />
                    )}
