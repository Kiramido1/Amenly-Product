...
def sanitize_input(text: str) -> str:
    """
    Strip HTML/script markup from free-text framework fields.

    M-10: this used to REJECT any input containing < > " ' & ; which blocked
    legitimate values like "AT&T Security Standard" or "Children's Online Privacy".
    We now strip markup and return the cleaned text instead of erroring. SQL injection
    is prevented by SQLAlchemy parameterization and XSS by output encoding on the client.
    """
    if not text:
        return text

    # Remove script blocks (with content), then any remaining HTML tags.
...
async def list_frameworks(
    ...
    framework_type: FrameworkType | None = Query(
        None, description="Filter by framework type (STANDARD, REGULATION, GUIDELINE)"
    ),
    category: FrameworkCategory | None = Query(None, description="Filter by category"),
    region: str | None = Query(
        None, description="Filter by region (e.g., 'United States', 'European Union')"
    ),
    is_mandatory: bool | None = Query(None, description="Filter by mandatory status"),
    search: str | None = Query(None, description="Search in framework name or description"),
) -> Any:
    """
    ...
    **Returns:**
    ...
    base_query = select(Framework)

    # If not superuser (we assume current RBAC model), filter by organization
    base_query = base_query.join(
        organization_frameworks, Framework.id == organization_frameworks.c.framework_id
    ...
    if search:
        ...
        base_query = base_query.where(
            or_(Framework.name.ilike(search_pattern), Framework.description.ilike(search_pattern))
        )
...
async def get_framework_types(current_user: Any = Depends(get_current_active_user)) -> Any:
    """
...
async def get_framework_categories(current_user: Any = Depends(get_current_active_user)) -> Any:
    """
    ...
    **Permissions:** Organization Admin only
    ...
    await db.execute(
        organization_frameworks.insert().values(
            organization_id=current_user.organization_id, framework_id=framework.id
        )
    )
...
async def update_framework(
    ...
    **Permissions:** Organization Admin only
    ...
    # SECURITY: the Framework row is shared across organizations via the junction
    # table. Editing it in place would tamper with every other tenant that uses it
    # (rename, flip is_mandatory, repoint official_url, etc.). Only allow edits when
    # this organization is the sole associated owner.
    assoc_count = await db.execute(
        select(func.count())
        .select_from(organization_frameworks)
        .where(organization_frameworks.c.framework_id == framework_id)
    )
    if assoc_count.scalar_one() > 1:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="This framework is shared with other organizations and cannot be edited.",
        )
...
async def delete_framework(
    ...
    **Permissions:** Organization Admin only
    ...
    # SECURITY: Framework is a shared/global catalog row referenced by many
    # organizations via the junction table. Calling db.delete(framework) here would
    # cascade into EVERY other organization's controls and assessments (irreversible
    # cross-tenant data loss). Only remove THIS organization's association; hard-delete
    # the catalog row solely when no other organization still references it.
    ...
    return {
        "success": True,
        "message": f"Framework '{framework_name}' removed from your organization",
    }
...
async def associate_frameworks(
    ...
    **Permissions:** Organization Admin only
    ...
    # request.framework_ids can be None, [], or [UUID]
    # request.framework_types can be None, [], or [FrameworkType]

    has_ids = request.framework_ids is not None and len(request.framework_ids) > 0
    has_types = request.framework_types is not None and len(request.framework_types) > 0

    if not has_ids and not has_types and not request.add_all:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if request.add_all:
        ...
        # Fix: ensure we use .value if they are enums or keep as strings if DB expects strings
        query = query.where(Framework.framework_type.in_(request.framework_types))
    else:
        # This case should not be reachable due to the check above, but for safety:
        return {
            "success": True,
            "message": "No frameworks to add",
            "data": {
                "added_count": 0,
                "skipped_count": 0,
                "total_frameworks": 0,
    ...
    existing_result = await db.execute(
        select(organization_frameworks.c.framework_id).where(
    ...
    for framework in frameworks_to_add:
        ...
        await db.execute(
            organization_frameworks.insert().values(
                organization_id=org_id, framework_id=framework.id
            )
        )
    ...
    total_result = await db.execute(
        select(func.count(organization_frameworks.c.framework_id)).where(
            organization_frameworks.c.organization_id == org_id
        )
    )
...
async def get_all_available_frameworks(
    ...
    framework_type: FrameworkType | None = Query(None, description="Filter by framework type"),
    category: FrameworkCategory | None = Query(None, description="Filter by category"),
    search: str | None = Query(None, description="Search in framework name or description"),
) -> Any:
    """
...
async def dissociate_framework(
    ...
    **Permissions:** Organization Admin only
    ...
    check_result = await db.execute(
        select(organization_frameworks).where(
            organization_frameworks.c.organization_id == org_id,
            organization_frameworks.c.framework_id == framework_id,
        )
    )
    ...
    fw_result = await db.execute(select(Framework).where(Framework.id == framework_id))
